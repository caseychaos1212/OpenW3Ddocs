...
* Category: ``mission``
* Indexed registrations: ``86``
* Source: ``Code/Scripts/Mission03.cpp``

DLS_Volcano_Active
------------------

DLS_Volcano_Active in Mission03.cpp initializes behavior when the object is created; responds to custom events; continues work on timer callbacks; watches enter or exit events; uses timers; sends custom events; creates explosions; starts conversations.
...
DLS_Volcano_Stumble in Mission03.cpp initializes behavior when the object is created; drives AI action commands.

* Source line: ``5546``
* Event hooks: ``Created``, ``Sound_Heard``, ``Action_Complete``
* Persistence hooks: none detected
* Key engine calls: ``Set_Innate_Take_Cover_Probability``, ``Set_Innate_Aggressiveness``, ``Action_Reset``, ``Get_Random_Int``, ``Action_Play_Animation``, ``Action_Goto``
* Summary source: ``heuristic``
...
M03_Alternate_Sam_Site in Mission03.cpp initializes behavior when the object is created; responds to custom events; continues work on timer callbacks; drives AI action commands; uses timers; creates or destroys objects.
...
M03_Ambient_Birdcall_Controller_JDG in Mission03.cpp initializes behavior when the object is created; responds to custom events; watches enter or exit events; sends custom events; creates or destroys objects.
...
M03_Chinook_Fodder_Creator in Mission03.cpp responds to custom events; continues work on timer callbacks; sends custom events; starts conversations.
...
* Event hooks: ``Created``, ``Custom``
* Persistence hooks: none detected
* Key engine calls: ``Find_Object``, ``Create_Sound``, ``Create_Object``, ``Set_Facing``, ``Attach_Script``, ``Send_Custom_Event``
* Summary source: ``heuristic``
...
* Persistence hooks: none detected
* Key engine calls: ``Set_Innate_Aggressiveness``, ``Start_Timer``, ``Action_Goto``, ``Get_Random``, ``Get_Position``, ``Get_Facing``, ``Action_Attack``, ``Find_Object``
* Summary source: ``heuristic``
...
M03_Destroyed_Chinook in Mission03.cpp reacts to destruction state; creates or destroys objects.

* Source line: ``3962``
* Event hooks: ``Killed``
* Persistence hooks: none detected
* Key engine calls: ``Find_Object``, ``Destroy_Object``, ``Create_Explosion_At_Bone``
* Summary source: ``heuristic``
...
M03_Destroyed_Turret in Mission03.cpp initializes behavior when the object is created; continues work on timer callbacks; uses timers; plays sounds.
...
* Persistence hooks: none detected
* Key engine calls: ``Enable_Hibernation``, ``Enable_Engine``, ``Get_Max_Health``, ``Get_Random``, ``Action_Attack``, ``Start_Timer``, ``Action_Reset``, ``Action_Goto``
* Summary source: ``heuristic``
...
* Event hooks: ``Created``, ``Custom``, ``Action_Complete``, ``Timer_Expired``
* Persistence hooks: none detected
* Key engine calls: ``Set_Num_Tertiary_Objectives``, ``Set_Wind``, ``Start_Timer``, ``Get_Random``, ``Add_Objective``, ``Find_Object``, ``Set_Objective_Radar_Blip_Object``, ``Set_Objective_HUD_Info``
* Summary source: ``heuristic``
...
M03_Protect_The_MCT in Mission03.cpp initializes behavior when the object is created.

* Source line: ``5764``
* Event hooks: ``Created``, ``Enemy_Seen``
* Persistence hooks: none detected
* Key engine calls: ``Is_A_Star``, ``Set_Innate_Soldier_Home_Location``
* Summary source: ``heuristic``
...
M03_Reinforce_Area in Mission03.cpp initializes behavior when the object is created; responds to custom events; continues work on timer callbacks; uses timers; sends custom events; creates or destroys objects.

* Source line: ``4385``
* Event hooks: ``Created``, ``Custom``, ``Timer_Expired``
* Persistence hooks: none detected
* Key engine calls: ``Start_Timer``, ``Send_Custom_Event``, ``Create_Object``, ``Set_Facing``, ``Attach_Script``
* Summary source: ``heuristic``
...
M03_Staged_Conversation_1 in Mission03.cpp responds to custom events; sends custom events.
...
RMV_Test_Big_Gun_Turning in Mission03.cpp initializes behavior when the object is created; responds to custom events; continues work on timer callbacks; reacts to destruction state; drives AI action commands; uses timers; sends custom events; creates or destroys objects.

* Source line: ``684``
* Event hooks: ``Created``, ``Killed``, ``Damaged``, ``Custom``, ``Sound_Heard``, ``Timer_Expired``
* Persistence hooks: none detected
* Key engine calls: ``Create_Object``, ``Attach_Script``, ``Send_Custom_Event``, ``Find_Object``, ``Shake_Camera``, ``Get_Random``, ``Get_Health``, ``Set_Health``
* Summary source: ``heuristic``
...
Sakura_Killed in Mission03.cpp initializes behavior when the object is created; reacts to destruction state; sends custom events; creates or destroys objects.
