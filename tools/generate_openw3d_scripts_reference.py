...
def main() -> int:
    ...
    parser.add_argument(
        "--source-revision",
        help="Reviewed source commit SHA, recorded as provenance without changing extraction",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
...
def render_reference_index(
    ...
    lines = [
        ...
        "",
        f"* Script registrations indexed: ``{catalog['script_count']}``",
        f"* Source files indexed: ``{catalog['file_count']}``",
        f"* Source revision: ``{catalog.get('source_revision', 'not recorded')}``",
...
def render_source_file_page(source_root: Path, source_file: str, entries: list[ScriptEntry]) -> str:
    ...
    lines = [
        ...
        f"* Indexed registrations: ``{len(entries)}``",
        f"* Source: ``{rel_source}``",
        "",
    ]
...
def indent_lines(lines: Iterable[str]) -> list[str]:
    return [f"   {line}".expandtabs(8).rstrip() if line else "" for line in lines]
...
def extract_scripts_from_file(path: Path, overrides: dict[str, dict[str, str]]) -> list[ScriptEntry]:
    ...
    code_text = mask_cpp_comments(active_text)
    scripts = []

    for match in DECLARE_SCRIPT_RE.finditer(code_text):
        name = match.group(1)
        parameter_description = decode_string_literals(match.group(2))
        class_body = extract_class_body(code_text, match.end())
...
def mask_cpp_comments(text: str) -> str:
    """Blank comments while preserving source positions and quoted literals."""
...
def preprocess_source(text: str) -> str:
    ...
    directive_lines = mask_cpp_comments(text).splitlines(keepends=True)
    stack: list[dict[str, object]] = []
    output = []

    for line, directive_line in zip(lines, directive_lines):
        directive = re.match(r"\s*#\s*(if|ifdef|ifndef|elif|else|endif)\b(.*)", directive_line)
        if directive:
...
def extract_preceding_comment(text: str, decl_start: int) -> str:
    ...
    # Disabled registrations are old code, not documentation for the next class.
    if DECLARE_SCRIPT_RE.search(raw):
        return ""
